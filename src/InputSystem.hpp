...
private:
	InputContext* GetContext(Epic::StringHash contextName) noexcept
	{
	...
	InputAction* GetAction(Epic::StringHash actionName) noexcept
	{
...
public:
	...
	auto Action(Epic::StringHash actionName) noexcept -> decltype(InputAction::Action)&
	{
		return GetAction(actionName)->Action;
	}

	// Activate an input context. If it does not exist, it will be created.
	void ActivateContext(Epic::StringHash contextName) noexcept
	{
	...
	void DeactivateContext(Epic::StringHash contextName) noexcept
	{
	...
	bool Bind(Epic::StringHash actionName, Epic::StringHash contextName, InputResolverPtr&& pResolver, bool clearFirst = false) noexcept
	{
		// Create the binding
	...
	// If 'clearFirst' is true, all other bindings for this Action will be cleared first.
	// Returns whether or not the Bind was successful.
	bool Bind(Epic::StringHash actionName, Epic::StringHash contextName, InputAction::Slot slot, InputResolverPtr&& pResolver, bool clearFirst = false) noexcept
	{
		// Create the binding
		auto pAction = GetAction(actionName);
	...
	// If 'clearFirst' is true, all other bindings for this Action will be cleared first.
	// Returns whether or not the Bind was successful.
	inline bool Bind(Epic::StringHash actionName, InputAction::Slot slot, InputResolverPtr&& pResolver, bool clearFirst = false) noexcept
	{
	...
	// The binding will occur in the first available slot.
	// Returns whether or not the Bind was successful.
	inline bool Bind(Epic::StringHash actionName, InputResolverPtr&& pResolver, bool clearFirst = false) noexcept
	{
